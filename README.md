...
一个安卓 APP：自动接收手机短信，并可在用户授权后监听 APP 通知；两类消息都按后台随“设备号校验”结果下发的**交易模式**规则解析：短信消息依次尝试所有短信模式（notify_type=0），APP 通知依次尝试所有 APP 通知模式（notify_type=1），每个模式依次执行“信息来源判断 → 交易信息判定正则 → 交易号提取正则”，直至某个模式命中；交易号与交易金额由模式正则从消息正文提取，充值卡号则在去除空格后的交易号上按模式正则提取；每条命中的消息都会保存本地记录并调用后台接口，同时按最近 1 小时记录标记为新消息、重复消息或重复交易号。所有本地记录会注明消息源自短信还是 APP 通知，并支持在 APP 内查看、按日期筛选以及按天数批量清除历史消息。APP 首次使用需先通过设备号校验，校验通过前不会启动短信/APP通知拦截功能。APP 支持多语言（中文、英语、法语、西班牙语、葡萄牙语）：APP 名称固定使用英文名（SMS Recharge Card Assistant），界面默认显示英文，首次打开的设备绑定页与主界面菜单“Language”均可选择语言，也可选择跟随系统语言，选择结果会被持久化保存；默认文案为英文（`res/values/strings.xml`，各语言翻译位于 `values-zh` / `values-fr` / `values-es` / `values-pt`）。本说明文档使用中文撰写。
